...
// MobileNet V2 fp16 model with 'nchw' input layout
export class MobileNetV27Nchw {
  constructor() {
    this.context_ = null;
    this.builder_ = null;
    this.graph_ = null;
    this.weightsUrl_ = './weights/mobilenetv2_fp16_nchw/';
    this.inputOptions = {
  ...
  async buildConv_(input, name, clip = true, options = {}) {
    ...
    if (clip) {
      ...
      return this.builder_.clamp(this.builder_.conv2d(
          input, weights, options), {minValue: 0, maxValue: 6});
    } else {
  ...
  async load(contextOptions) {
    ...
    const data = this.builder_.input('input', {dataType:
        this.inputOptions.dataType, dimensions:
    ...
    const conv4 = await this.buildConv_(conv3, '97', false,
