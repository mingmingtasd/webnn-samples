...
export class EfficientNetFP16Nchw {
  ...
  async buildGemm_(input, name) {
    ...
    const options =
        {c: this.builder_.reshape(bias, [1, 1000])};
  ...
  async buildBottleneck_(input, blockName, group, pad = 1) {
    ...
    const conv2 = await this.buildConv_(conv1, '1', blockName, true,
  ...
  async load(contextOptions) {
    ...
    const conv2 = await this.buildConv_(conv1, '1', '0', true,
        {groups: 32, padding: [1, 1, 1, 1]});
    const conv3 = await this.buildConv_(conv2, '2', '0');

    // Block 1
    const conv4 = await this.buildConv_(conv3, '0', '1', true);
    const conv5 = await this.buildConv_(conv4, '1', '1', true,
        {groups: 144, padding: [0, 1, 0, 1], strides: [2, 2]});
    const conv6 = await this.buildConv_(conv5, '2', '1');

    // Block 2~4
    const bottleneck4 = await this.buildBottlenecks_(conv6,
        ['2', '3', '4'], 192);

    // Block 5
    const conv7 = await this.buildConv_(bottleneck4, '0', '5', true);
    const conv8 = await this.buildConv_(conv7, '1', '5', true,
        {groups: 192, padding: [1, 2, 1, 2], strides: [2, 2]});
    const conv9 = await this.buildConv_(conv8, '2', '5');

    // Block 6~8
    const bottleneck8 = await this.buildBottlenecks_(conv9,
        ['6', '7', '8'], 336, 2);

    // Block 9
    const conv10 = await this.buildConv_(bottleneck8, '0', '9', true);
    const conv11 = await this.buildConv_(conv10, '1', '9', true,
        {groups: 336, padding: [0, 1, 0, 1], strides: [2, 2]});
    const conv12 = await this.buildConv_(conv11, '2', '9');

    // Block 10~14
    const bottleneck14 = await this.buildBottlenecks_(conv12,
        ['10', '11', '12', '13', '14'], 672);

    // Block 15
    const conv13 = await this.buildConv_(bottleneck14, '0', '15', true);
    const conv14 = await this.buildConv_(conv13, '1', '15', true,
        {groups: 672, padding: [2, 2, 2, 2]});
    const conv15 = await this.buildConv_(conv14, '2', '15');
    ...
    const conv17 = await this.buildConv_(conv16, '1', '21', true,
        {groups: 960, padding: [1, 2, 1, 2], strides: [2, 2]});
    const conv18 = await this.buildConv_(conv17, '2', '21');

    // Block 22~28
    const bottleneck28 = await this.buildBottlenecks_(conv18,
        ['22', '23', '24', '25', '26', '27', '28'], 1632, 2);

    // Block 29
    const conv19 = await this.buildConv_(bottleneck28, '0', '29', true);
    const conv20 = await this.buildConv_(conv19, '1', '29', true,
        {groups: 1632, padding: [1, 1, 1, 1]});
    const conv21 = await this.buildConv_(conv20, '2', '29');

    const conv22 = await this.buildConv_(conv21, '0', '', true);
    const pool1 = this.builder_.averagePool2d(conv22);
